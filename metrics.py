...
def turbine_scores(pred, gt, raw_data, examine_len, stride=1):
    ...
    for i in range(0, cnt_sample, stride):
        ...
        _mae, _rmse = regressor_scores(prediction[-examine_len:] ,
                                       targets[-examine_len:] )
        if not np.isnan(_mae) and not np.isnan(_rmse):
            maes.append(_mae)
            rmses.append(_rmse)
    avg_mae = np.array(maes).mean() if maes else np.nan
    avg_rmse = np.array(rmses).mean() if rmses else np.nan
...
def regressor_detailed_scores(predictions, gts, raw_df_lst, capacity,
    ...
    """
    all_mae, all_rmse = [], []
    actual_capacity = min(capacity, len(predictions))
